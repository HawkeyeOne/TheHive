...
case class DemistoInstanceConfig(name: String, url: String, key: String, caseTemplate: Option[String], artifactTags: Seq[String])
object DemistoInstanceConfig {
  def apply(name: String, defaultCaseTemplate: Option[String], configuration: Configuration): Option[DemistoInstanceConfig] =
    for {
      url ← configuration.getString("url")
      key ← configuration.getString("key")
      tags = configuration.getStringSeq("tags").getOrElse(Nil)
    } yield DemistoInstanceConfig(name, url, key, configuration.getString("caseTemplate") orElse defaultCaseTemplate, tags)
}
case class DemistoConfig(truststore: Option[Path], interval: FiniteDuration, instances: Seq[DemistoInstanceConfig]) {

  def this(configuration: Configuration, defaultCaseTemplate: Option[String]) = this(
    configuration.getString("demisto.cert").map(p ⇒ Paths.get(p)),
    configuration.getMilliseconds("demisto.interval").map(_.millis).getOrElse(1.hour),
    for {
      cfg ← configuration.getConfig("demisto").toSeq
      key ← cfg.subKeys
      c ← Try(cfg.getConfig(key)).toOption.flatten.toSeq
      mic ← DemistoInstanceConfig(key, defaultCaseTemplate, c)
    } yield mic)

  @Inject def this(configuration: Configuration) =
    this(configuration, configuration.getString("demisto.caseTemplate"))
}

@Singleton
class DemistoSrv @Inject() (
    demistoConfig: DemistoConfig,
    demistoModel: DemistoModel,
    caseSrv: CaseSrv,
  ...
  val ws = {
    val config = demistoConfig.truststore match {
  ...
  private[demisto] def getInstanceConfig(name: String): Future[DemistoInstanceConfig] = demistoConfig.instances
    .find(_.name == name)
    .fold[Future[DemistoInstanceConfig]](Future.failed(NotFoundError(s"""Configuration of DEMISTO server "${name}" not found"""))) { instanceConfig ⇒
      Future.successful(instanceConfig)
    }

  def initScheduler() = {
    val task = system.scheduler.schedule(0.seconds, demistoConfig.interval) {
      log.info("Update of DEMISTO events is starting ...")
      userSrv
        .inInitAuthContext { implicit authContext ⇒
          update().andThen { case _ ⇒ tempSrv.releaseTemporaryFiles() }
        }
        .onComplete {
          case Success(demistos) ⇒
            val successDemisto = demistos.flatMap {
              case Success(m) ⇒ Seq(m)
              case Failure(t) ⇒ log.warn("Demisto import fail", t); Nil
            }
            log.info(s"${successDemisto.size} DEMISTO event(s) updated")
          case Failure(error) ⇒ log.error("Update DEMISTO events error :", error)
        }
      ()
    }
    lifecycle.addStopHook { () ⇒
      log.info("Stopping DEMISTO fetching ...")
      task.cancel()
      Future.successful(())
    }
  }
  ...
  /**
   * Get list of all DEMISTO event (without attributes)
   */
  def getEvents: Future[Seq[DemistoEvent]] = {
    Future
      .traverse(demistoConfig.instances) { c ⇒
        getEvents(c).recoverWith {
          case t ⇒
            log.warn("Retrieve DEMISTO event list failed", t)
            Future.failed(t)
        }
      }
      .map(_.flatten)
  ...
  def getEvents(instanceConfig: DemistoInstanceConfig): Future[Seq[DemistoEvent]] = {
    ws.url(s"${instanceConfig.url}/events/index")
      ...
      .map { response ⇒
        ...
        val events = eventJson.flatMap { j ⇒
          j.asOpt[DemistoEvent]
            .map(_.copy(serverId = instanceConfig.name))
            .orElse {
              log.warn(s"DEMISTO event can't be parsed\n$j")
              None
            }
        ...
        if (eventJsonSize != eventsSize)
          log.warn(s"DEMISTO returns $eventJsonSize events but only $eventsSize contain valid data")
        events
      }
  }
  ...
  def extractMalwareAttachment(file: FileInputValue)(implicit authContext: AuthContext): FileInputValue = {
    ...
    try {
      ...
      (for {
        ...
        tempFile = tempSrv.newTemporaryFile("demisto_malware", file.name)
  ...
  def downloadAttachment(instanceConfig: DemistoInstanceConfig, attachmentId: String)(implicit authContext: AuthContext): Future[FileInputValue] = {
      ...
      .flatMap {
        case response if response.headers.status != 200 ⇒
          ...
          response.body.runWith(Sink.headOption).flatMap { body ⇒
            ...
            log.warn(s"DEMISTO attachment $attachmentId can't be downloaded (status $status) : $message")
            Future.failed(InternalError(s"DEMISTO attachment $attachmentId can't be downloaded (status $status)"))
          }
        case response ⇒ Future.successful(response)
      }
      .flatMap { response ⇒
        val tempFile = tempSrv.newTemporaryFile("demisto_attachment", attachmentId)
        response.body
  ...
  def update()(implicit authContext: AuthContext): Future[Seq[Try[Demisto]]] = {
    def getEventFields(event: DemistoEvent) = {
    ...
    def getAndSortDemistoFromEvents(events: Seq[DemistoEvent]): Future[Seq[Either[(Demisto, Fields), Fields]]] = flatFutureSeq(
      events.map { event ⇒
        val fields = getEventFields(event)
        getDemisto(event._id)
          .map {
            case demisto if (demisto.publishDate() before event.publishDate) ⇒
              demisto.eventStatus() match {
                case EventStatus.Ignore | EventStatus.Imported if demisto.follow() ⇒ Seq(Left(demisto → fields.set("eventStatus", "Update").unset("_id")))
                case _                                                             ⇒ Seq(Left(demisto → fields.unset("_id")))
              }
            case _ ⇒ Seq.empty
          }
          .recover {
            case _: NotFoundError ⇒ Seq(Right(fields))
            case error            ⇒ log.error("DEMISTO error", error); Seq.empty
          }
      })
    ...
    def getSuccessDemistoAndCase(demisto: Seq[Try[Demisto]]): Future[Seq[(Demisto, Case)]] = {
      val successDemisto = demisto.collect {
        case Success(m) ⇒ m
      }
      Future
        .traverse(successDemisto) { demisto ⇒
          caseSrv.get(demisto.id).map(demisto → _)
        }
        // remove deleted and merged cases
        .map {
          _.filter {
            case (demisto, caze) ⇒ caze.status() != CaseStatus.Deleted && caze.resolutionStatus != CaseResolutionStatus.Duplicated
          }
        }
    }

    /* for all demisto servers, retrieve events */
    getEvents
      /* sort events into : case must be updated (Left) and case must be created (Right) */
      .flatMap(getAndSortDemistoFromEvents)
      .map(partitionEither)
      .flatMap {
        case (updates, creates) ⇒
          for {
            updatedDemisto ← updateSrv(updates)
            createdDemisto ← createSrv[DemistoModel, Demisto](demistoModel, creates)
            demisto = updatedDemisto ++ createdDemisto
            importedDemisto ← getSuccessDemistoAndCase(updatedDemisto)
            // update case status
            _ ← caseSrv.bulkUpdate(importedDemisto.map(_._2.id), Fields.empty.set("status", CaseStatus.Open.toString))
            // and import DEMISTO attributes
            _ ← Future.traverse(importedDemisto) {
              case (m, c) ⇒
  ...
  def pullEvent(instanceConfig: DemistoInstanceConfig, eventId: Long): Future[JsValue] = {
    ws
  ...
  def createCase(demistoId: String)(implicit authContext: AuthContext): Future[(Case, Seq[Try[Artifact]])] =
    getDemisto(demistoId).flatMap(demisto ⇒ createCase(demisto))

  def createCase(demisto: Demisto)(implicit authContext: AuthContext): Future[(Case, Seq[Try[Artifact]])] = {
    demisto.caze() match {
      case Some(id) ⇒ caseSrv.get(id).map(_ → Nil)
      case None ⇒
        for {
          instanceConfig ← getInstanceConfig(demisto.serverId())
          ...
          attributes ← getAttributes(demisto)
          links = attributes
            .collect { case a if a.tpe == "link" ⇒ a.value }
          linkDescription = if (links.isEmpty) "" else
            links.mkString("\n\nLinks attributes :\n\n - ", "\n - ", "")
          caseTitle = (caseTemplate.flatMap(_.titlePrefix()).getOrElse("") + s" #${demisto.eventId()} " + demisto.info()).trim
          caseTlp = demisto.tags()
            .map(_.toLowerCase)
            .collectFirst {
              case "tlp:white" ⇒ 0L
              case "tlp:green" ⇒ 1L
            ...
            .getOrElse(2L)
          caseTags = demisto.tags().filterNot(_.toLowerCase.startsWith("tlp:")) ++ caseTemplate.fold[Seq[String]](Nil)(_.tags()) :+ s"src:${demisto.org()}"
          ...
          caseFields = Json.obj(
            "title" → caseTitle,
            "description" → s"Imported from DEMISTO Event #${demisto.eventId()}, created at ${demisto.date()}${linkDescription}",
            "tlp" → caseTlp,
            "severity" → demisto.threatLevel(),
            "flag" → caseFlag,
            "tags" → caseTags,
            "metrics" → caseMetrics,
            "tasks" → caseTasks)
          caze ← caseSrv.create(Fields(caseFields))
          _ ← setCaseId(demisto.id, caze.id)
          artifacts ← importAttributes(demisto, caze)
        } yield (caze, artifacts)
    }
  }

  def getDemisto(demistoId: String): Future[Demisto] =
    getSrv[DemistoModel, Demisto](demistoModel, demistoId)

  def find(queryDef: QueryDef, range: Option[String], sortBy: Seq[String]): (Source[Demisto, NotUsed], Future[Long]) = {
    import org.elastic4play.services.QueryDSL._
    findSrv[DemistoModel, Demisto](demistoModel, and(queryDef, not("eventStatus" in ("Ignore", "Imported"))), range, sortBy)
  }

  def stats(queryDef: QueryDef, aggs: Seq[Agg]) = findSrv(demistoModel, queryDef, aggs: _*)

  def ignoreEvent(demistoId: String)(implicit authContext: AuthContext) = {
    updateSrv[DemistoModel, Demisto](demistoModel, demistoId, Fields(JsObject(Seq("eventStatus" → JsString("Ignore")))))
  }

  def setFollowEvent(demistoId: String, follow: Boolean)(implicit authContext: AuthContext) = {
    updateSrv[DemistoModel, Demisto](demistoModel, demistoId, Fields(JsObject(Seq("follow" → JsBoolean(follow)))))
  }

  def setCaseId(demistoId: String, caseId: String)(implicit authContext: AuthContext) = {
    updateSrv[DemistoModel, Demisto](demistoModel, demistoId, Fields(Json.obj("case" → caseId, "eventStatus" → EventStatus.Imported)))
  }

  def importAttributes(demistoId: String)(implicit authContext: AuthContext): Future[Seq[Try[Artifact]]] = {
    getSrv[DemistoModel, Demisto](demistoModel, demistoId)
      .flatMap { demisto ⇒ importAttributes(demisto) }
  }

  def importAttributes(demisto: Demisto)(implicit authContext: AuthContext): Future[Seq[Try[Artifact]]] = {
    for {
      caseId ← demisto.caze() match {
        case Some(c) ⇒ Future.successful(c)
        case None    ⇒ Future.failed(NotFoundError(s"No case defined for DEMISTO event ${demisto.id}"))
      }
  ...
  def importAttributes(demisto: Demisto, caze: Case)(implicit authContext: AuthContext): Future[Seq[Try[Artifact]]] = {
    for {
      instanceConfig ← getInstanceConfig(demisto.serverId())
      demistoAttributes ← getAttributes(demisto)
      artifactTags = JsString(s"src:${demisto.org()}") +: JsArray(instanceConfig.artifactTags.map(JsString))
      attributes ← Future.sequence(demistoAttributes
        .map {
  ...
  def getAttributes(demisto: Demisto): Future[Seq[DemistoAttribute]] = {
    for {
      instanceConfig ← getInstanceConfig(demisto.serverId())
      event ← pullEvent(instanceConfig, demisto.eventId())
    } yield (event \ "Event" \ "Attribute").as[Seq[DemistoAttribute]]
  }

  def convertAttribute(demistoAttribute: DemistoAttribute)(implicit authContext: AuthContext): Seq[JsObject] = {
    val data = demistoAttribute.value
    val filenameExtractor = "filename\\|(.*)".r
    val fields = JsObject(Seq(
      "data" → JsString(data),
      "dataType" → JsString(demistoAttribute.tpe),
      "message" → JsString(demistoAttribute.comment)))
    demistoAttribute.tpe match {
      case filenameExtractor(hashType) ⇒
        val Array(file, hash, _*) = (data + "|").split("\\|", 3).map(JsString)
        Seq(
          fields + ("data" → file) + ("dataType" → JsString("filename")) + ("message" → JsString(demistoAttribute.comment + s"\n$hashType:" + hash)),
          fields + ("data" → hash) + ("dataType" → JsString("hash")) + ("tags" → JsArray(Seq(JsString(hashType)))) + ("message" → JsString(demistoAttribute.comment + s"\nfilename:" + file)))
      ...
      case "domain|ip" ⇒ /* A domain name and its IP address (as found in DNS lookup) separated by a | (no spaces) */
        ...
        Seq(
          fields + ("data" → domain) + ("dataType" → JsString("domain")) + ("message" → JsString(demistoAttribute.comment + "\nip:" + ip)),
          fields + ("data" → ip) + ("dataType" → JsString("ip")) + ("message" → JsString(demistoAttribute.comment + "\ndomain:" + domain)))
      ...
      case "regkey|value" ⇒ /* Registry value + data separated by | */
        ...
        Seq(
          fields + ("data" → regkey) + ("dataType" → JsString("other")) + ("tags" → JsArray(Seq(JsString("regkey")))) + ("message" → JsString(demistoAttribute.comment + "\nvalue:" + value)),
          fields + ("data" → value) + ("dataType" → JsString("other")) + ("tags" → JsArray(Seq(JsString("regkey-value")))) + ("message" → JsString(demistoAttribute.comment + "\nkey:" + regkey)))
