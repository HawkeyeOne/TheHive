...
object JsonFormat {
  ...
  implicit val eventReads = Reads(json ⇒
    ...
    } yield DemistoEvent(uuid, "", eventId.toLong, org, info, tags, date, publishDate, attrCount, threatLevel.toInt, analysis.toInt))

  implicit val eventWrites = Json.writes[DemistoEvent]
  ...
  implicit val attributeReads = Reads(json ⇒
    ...
    } yield DemistoAttribute(id, tpe, category, uuid, eventId.toLong, date, comment, value))
}